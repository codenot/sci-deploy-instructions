...
| `us-rak-2` | `ALLXTNK43BFIUO` | Ubuntu 24.04 | 美国节点 | Remnawave Node、Komari Agent |
| `us-dedirock` | `dedirock-725800246` | Ubuntu 24.04 | 美国节点 | Remnawave Node、Komari Agent |
| `jp` | `jp.codermb.com` | Ubuntu 24.04 | 日本节点 | Remnawave Node、Komari Agent |
...
| Tailscale | 系统包 | systemd 服务 `tailscaled` | `41641/udp`，已作为 `hk-lc` 自建 Headscale 的客户端接入 |

### 注意事项

- `kr` 是当前 Remnawave Panel 所在机器，也是一个 Remnawave Node。Panel 访问同机 Node 时不要在面板里填 `127.0.0.1`，应使用 Docker 网关或宿主机可达地址。
- `kr` 的 Tailscale 已从官方控制面切换到 `hk-lc` 自建 Headscale；客户端登录入口使用 `<HEADSCALE_DOMAIN>:<HEADSCALE_HTTPS_PORT>`，不要把真实域名或 preauth key 写入仓库。
- 当前 `kr` 到 `hk-lc` 的 Tailscale 数据面可经 DERP 中继连通，直连仍需继续检查两端 `41641/udp`、云安全组和 NAT 映射。
...
- Komari Server 当前固定使用 `ghcr.io/komari-monitor/komari:1.3.0`；管理凭据和自动发现密钥只保存在 `/opt/komari` 且权限为 `0600`；当前启用 `Komari Next 1.4.18` 主题，主题文件位于 `/opt/komari/data/theme/next`；已为全部现有及未来节点启用北京、上海、广州三地电信/联通/移动共 9 个 ICMP 延迟任务，周期为 60 秒。
- Remnawave `Default-Squad` 只关联公共 `VLESS-Reality-25080`；`Private-Squad` 关联 `VLESS-Reality-443`、香港中转 `25083` 和日本中转 `25084`，只向明确加入该 Squad 的用户展示对应 Host。
...
- Docker：`active`
- Tailscale：`active`
- NTP：`systemd-timesyncd` 已启用并同步；可靠时间源配置位于 `/etc/systemd/timesyncd.conf.d/10-reliable-ntp.conf`。
...
| 服务 | 部署目录 | 运行形态 | 监听/暴露 |
|---|---|---|---|
| Remnawave Node | `/opt/remnawave-node` | Docker 容器 `remnawave-node`，`network_mode: host` | `*:2222` 控制 API、`*:25080`、`*:25083`、`*:443` 代理入站 |
...
- `hk-lc` 自身已作为 Headscale/Tailscale 节点接入，客户端接入时使用 `<HEADSCALE_DOMAIN>:<HEADSCALE_HTTPS_PORT>`。
...
| 服务 | 部署目录 | 运行形态 | 监听/暴露 |
|---|---|---|---|
| Remnawave Node | `/opt/remnawave-node` | Docker 容器 `remnawave-node`，`network_mode: host` | `*:2222` 控制 API、`*:443` VLESS/Reality 私有入站、`*:25180` Shadowsocks 2022 中继入站 |
...
- `vn` 的 Remnawave Node 已接入 `kr` 的 Remnawave Panel，直连 Host 使用只分配给 `vn-node` 和 `hjus-node` 的 `VLESS-Reality-443` 入站，端口为 `443`，并由 `Private-Squad` 控制订阅可见性。
- `25180/tcp` 和 `25180/udp` 的 Shadowsocks 2022 入站由 Remnawave `Default-Profile` 下发且只关联 `vn-node`；`vn` 只运行 Remnawave Node 的 `rw-core`。旧独立 Xray 部署已停止并归档为 `/opt/remnawave-s2s.disabled-<TIMESTAMP>`。
...
- Remnawave Panel 中节点名称为 `us-node`，关联 `Default-Profile` 的 `VLESS-Reality-25080` 入站；订阅 Host 为 `美国 - RackNerd`，绑定 `VLESS-Reality-25080`；Komari 中节点名称为 `us`。
...
- Remnawave Panel 中节点名称为 `us-rak-1-node`，只关联 `Default-Profile` 的 `VLESS-Reality-25080` 入站；订阅 Host 为 `美国 - RAK 1`。Komari 中节点名称为 `us-rak-1`。
- `2222/tcp` 由 `remnawave-node-firewall.service` 限制为只允许 `kr` Panel 固定出口访问，IPv6 对该端口直接拒绝；云安全组也应维持相同策略。
- `25080/tcp` 是正式 VLESS/Reality 代理入站，需要在云安全组开放。
- 初始 iptables/ip6tables 备份位于 `/root/pre-remnawave-backup/<TIMESTAMP>/`；Remnawave Panel 变更前数据库备份位于 `/opt/remnawave/backup/remnawave-before-us-rak-nodes-<TIMESTAMP>.dump`。
...
- Remnawave Panel 中节点名称为 `us-rak-2-node`，只关联 `Default-Profile` 的 `VLESS-Reality-25080` 入站；订阅 Host 为 `美国 - RAK 2`。Komari 中节点名称为 `us-rak-2`。
- `2222/tcp` 由 `remnawave-node-firewall.service` 限制为只允许 `kr` Panel 固定出口访问，IPv6 对该端口直接拒绝；云安全组也应维持相同策略。
- `25080/tcp` 是正式 VLESS/Reality 代理入站，需要在云安全组开放。
- 初始 iptables/ip6tables 备份位于 `/root/pre-remnawave-backup/<TIMESTAMP>/`；Remnawave Panel 变更前数据库备份位于 `/opt/remnawave/backup/remnawave-before-us-rak-nodes-<TIMESTAMP>.dump`。
...
- Remnawave Panel 中节点名称为 `us-dedirock-node`，只关联 `Default-Profile` 的 `VLESS-Reality-25080` 入站；订阅 Host 为 `美国 - DediRock`。Komari 中节点名称为 `us-dedirock`。
- `2222/tcp` 由 `remnawave-node-firewall.service` 限制为只允许 `kr` Panel 固定出口访问，IPv6 对该端口直接拒绝；云安全组也应维持相同策略。
- `25080/tcp` 是正式 VLESS/Reality 代理入站，需要在云安全组开放。
- 初始 iptables、ip6tables 和 sysctl 备份位于 `/root/pre-remnawave-backup/<TIMESTAMP>/`；Remnawave 与 Komari 数据库变更前备份分别位于各自 `/opt/<service>/backup/` 目录。
...
- Docker：`active`，Engine `29.6.2`。
- TCP 拥塞控制：`bbr`；默认 qdisc：`fq`。
- 机器内存约 1 GiB，已配置 2 GiB `/swapfile` 并写入 `/etc/fstab`。

### 已部署服务

| 服务 | 部署目录 | 运行形态 | 监听/暴露 |
|---|---|---|---|
| Remnawave Node | `/opt/remnawave-node` | Docker 容器 `remnawave-node`，`network_mode: host` | `*:2222` 控制 API、`*:25080`、`*:25084` 代理入站 |
| Komari Agent | `/opt/komari-agent` | systemd 服务 `komari-agent` | 主动连接 `kr` Komari HTTPS 入口；Web SSH 已禁用 |

### 注意事项

- Remnawave Panel 中节点名称为 `jp-node`，关联默认 Profile 的 `VLESS-Reality-25080` 正式入站和仅分配给本节点的 `VLESS-Reality-VN-via-JP-25084` 中继入站；订阅 Host 包含 `日本 - JP` 和“越南 - 日本中转”；Komari 通过自动发现注册。
- `2222/tcp` 由 `remnawave-node-firewall.service` 在宿主机 `INPUT` 限制为只允许 `kr` Panel 固定出口访问；IPv6 对该端口直接拒绝。云安全组也应保持相同入口策略。
- `25080/tcp` 是日本直连 VLESS/Reality 入站，`25084/tcp` 是“越南 - 日本中转”入口；两者都需要在云安全组中开放。
- 防火墙只管理入口；不要给 `jp` 添加 `OUTPUT`、容器网段、Docker 网桥或远端目的地址出口白名单。
...
- `gz` 的 Tailscale 已从官方控制面切换到 `hk-lc` 自建 Headscale；客户端登录入口使用 `<HEADSCALE_DOMAIN>:<HEADSCALE_HTTPS_PORT>`，不要把真实域名或 preauth key 写入仓库。
- `gz` 自建 DERP 已发布到 `hk-lc` Headscale 的 DERP map；客户端优先通过 `gz` DERP relay 中继时，继续保持上游 `33443/tcp` 不直接暴露公网。
